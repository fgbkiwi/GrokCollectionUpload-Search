...
logger = logging.getLogger("PrecedentSifter")
...
class XAIClient:
    ...
    def __init__(self, api_key: str, management_key: str = ""):
        ...
        self.headers = {
            ...
            "User-Agent": "PrecedentSifter/1.0"
        }
...
class PrecedentSifterApp(QMainWindow):
    """Aplicação principal de busca de precedentes."""

    INPUT_BASE_HEIGHT = 28
    INPUT_MAX_MULTIPLIER = 8
    RESPONSE_BASE_HEIGHT = 75
    RESPONSE_MAX_MULTIPLIER = 4
    COPY_CHAT_IS_MARKDOWN = False
    ...
    def __init__(self):
        ...
        self.setWindowTitle("Kiwi-Sifter")
    ...
    def _load_toolbar_icon(self, file_name: str, fallback: QStyle.StandardPixmap) -> QIcon:
        """Carrega ícone customizado da raiz do projeto com fallback padrão do sistema."""
        icon_path = Path(__file__).resolve().parent / file_name
        if icon_path.exists():
            return QIcon(str(icon_path))
        logger.warning(f"Ícone não encontrado: {icon_path}")
        return self.style().standardIcon(fallback)

    def _create_brand_logo_widget(self) -> QLabel:
        """Cria widget de logo da marca para o topo direito da UI."""
        logo_label = QLabel()
        logo_label.setObjectName("brandLogo")
        logo_label.setToolTip("Kiwi-Sifter")

        logo_path = Path(__file__).resolve().parent / "ChatGPT5.5-generated_Kiwi-Sifter_logo_cropped.png"
        if logo_path.exists():
            ...
            logger.warning(f"Logo inválido (falha ao carregar pixmap): {logo_path}")
        else:
            logger.warning(f"Logo não encontrado: {logo_path}")

        logo_label.setText("Kiwi-Sifter")
        logo_label.setStyleSheet("font-weight: 700; font-size: 20px; padding: 0 10px;")
    ...
    def create_ui(self):
        ...
        clear_button.setIcon(
            self._load_toolbar_icon(
                "Gemini_Generated_Image_clean_chat_cropped.png",
        ...
        copy_icon_name = (
            "Gemini_Generated_Image_copy_in_markdown_cropped.png"
            if self.COPY_CHAT_IS_MARKDOWN
            else "Gemini_Generated_Image_copy_in-rtf_cropped.png"
        )
        copy_button.setIcon(
            self._load_toolbar_icon(
                copy_icon_name,
        ...
        attach_button.setIcon(
            self._load_toolbar_icon(
                "Gemini_Generated_Image_upload_pdf_or_md_cropped.png",
        ...
        settings_button.setIcon(
            self._load_toolbar_icon(
                "Gemini_Generated_Image_settings_cropped.png",
                QStyle.StandardPixmap.SP_ComputerIcon,
            )
        )
        settings_button.clicked.connect(self.open_settings)

        for button in (clear_button, copy_button, attach_button, settings_button):
        ...
        toolbar_layout.addWidget(
            self._create_brand_logo_widget(),
            0,
